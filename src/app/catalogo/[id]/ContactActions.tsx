...
export default function ContactActions({ vehicleId, phone, brand, model }: ContactActionsProps) {
  const [showPhone, setShowPhone] = useState(false);
  const [showWarning, setShowWarning] = useState(false);

  const trackAction = async (action: "whatsapp" | "phone") => {
    try {
      await fetch(`/api/vehicles/${vehicleId}/track`, {
        method: "POST",
  ...
  return (
    <>
        ...
        <div className={styles.warningOverlay} onClick={() => setShowWarning(false)}>
          <div className={styles.warningModal} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={styles.warningActions}>
              ...
              <button className={styles.btnWarningConfirm} onClick={handleWhatsAppConfirm}>
                Entendido, contactar
              </button>
            </div>
          </div>
      ...
      <div className={styles.container}>
        <button className={styles.btnWhatsapp} onClick={() => setShowWarning(true)}>
        ...
        <div className={styles.phoneSection}>
          {showPhone ? (
            <div className={styles.phoneDisplay}>
              <span className={styles.phoneLabel}>Teléfono:</span>
              <a href={`tel:${phone}`} className={styles.phoneLink}>{phone}</a>
            </div>
          ) : (
            <button className={styles.btnShowPhone} onClick={handleShowPhone}>
              Ver teléfono
            </button>
          )}
        </div>
